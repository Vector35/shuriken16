...
AddMapDialog::AddMapDialog(QWidget* parent, shared_ptr<Project> project): QDialog(parent), m_project(project)
{
	...
	layout->addWidget(new QLabel("Map Size:"));
	...
	connect(okButton, &QPushButton::clicked, this, &AddMapDialog::OKButton);
	connect(cancelButton, &QPushButton::clicked, this, &AddMapDialog::reject);

	setLayout(layout);
}


void AddMapDialog::OKButton()
{
	...
	{
		QMessageBox::critical(this, "Error", "Map width is invalid.");
		return;
	}
	...
	m_map = make_shared<Map>(width, height, tileWidth, tileHeight, tileDepth);
	m_map->SetName(name);

	if (!m_project->AddMap(m_map))
	{
		QMessageBox::critical(this, "Error", "Map name is already used. Please choose a different name.");
		return;
	}
