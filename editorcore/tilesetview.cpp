	...
	EditorView(parent), m_mainWindow(parent), m_project(project), m_tileSet(tileSet)
{
	...
	QScrollArea* previewAndPaletteScrollArea = new QScrollArea(this);
	QWidget* previewAndPalette = new QWidget(this);
	QVBoxLayout* previewAndPaletteLayout = new QVBoxLayout();
	previewAndPaletteLayout->setContentsMargins(8, 8, 0, 8);
	QHBoxLayout* previewLayout = new QHBoxLayout();
	m_preview = new TileSetPreviewWidget(previewAndPalette, this, parent, project, tileSet);
	m_editor->SetPreviewWidget(m_preview);
	previewLayout->addWidget(m_preview);
	previewLayout->addStretch(1);
	previewAndPaletteLayout->addLayout(previewLayout);
	...
	previewAndPaletteLayout->addWidget(m_previewAnim);
	m_anim = new TileSetAnimationWidget(previewAndPalette, m_editor, parent, tileSet);
	m_editor->SetAnimationWidget(m_anim);
	previewAndPaletteLayout->addWidget(m_anim);
	m_palettes = new TileSetPaletteWidget(previewAndPalette, this, parent, project, tileSet);
	previewAndPaletteLayout->addWidget(m_palettes);
	previewAndPaletteLayout->addStretch(1);
	previewAndPalette->setLayout(previewAndPaletteLayout);
	previewAndPaletteScrollArea->setHorizontalScrollBarPolicy(Qt::ScrollBarAlwaysOff);
	previewAndPaletteScrollArea->setWidgetResizable(true);
	previewAndPaletteScrollArea->setAlignment(Qt::AlignTop);
	previewAndPaletteScrollArea->setWidget(previewAndPalette);
	previewAndPaletteScrollArea->horizontalScrollBar()->setEnabled(false);
	contentLayout->addWidget(previewAndPaletteScrollArea);
